...
__device__ void process_pixel(unsigned char* src_image, unsigned char* dst_image, int width, int x, int y) {
    ...
    int padding = (0 * width + x)*image_channels;
    int end = (y+1)*(width)*image_channels-padding;
    dst_image[start]=src_image[end-4];
    dst_image[start+1]=src_image[end-3];
    dst_image[start+2]=src_image[end-2];
    dst_image[start+3]=src_image[end-1];
}
